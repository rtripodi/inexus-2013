...
#else
  #include "Wiring.h"
#endif		// provides access to delay() and delayMicroseconds()
#endif
  ...
  unsigned char emitterPin, unsigned char type)
{
	...
	if (emitterPin < 30)				// port A
	{
		_emitterBitmask = 1 << (emitterPin - 22);
		_emitterPORT = &PORTA;
		_emitterDDR = &DDRA;
	}
	else if (emitterPin < 38)		// port C
	{
		_emitterBitmask = 1 << (emitterPin - 30);
		_emitterPORT = &PORTC;
		_emitterDDR = &DDRC;
	}
	else
	...
	unsigned char numSensors, unsigned int timeout, unsigned char emitterPin)
{
	...
	for (i = 0; i < _numSensors; i++)
	{
		if (pins[i] < 30)			// port A
		{
			_bitmask[i] = 1 << (pins[i] - 22);
			_portAMask |= _bitmask[i];
			_register[i] = &PINA;
		}
		else if (pins[i] < 38)		// port C
		{
			_bitmask[i] = 1 << (pins[i] - 30);
